...
// 안 쓰고 있던것
function fn_cOrderQuick(f) {
	f.action = 'cOrderQuick';
	f.submit();
}
			...
			<c:otherwise>
				<form id="f">
					<div class="checkDelete">
						<label class="chk_all"><input type="checkbox" id="chk_all" checked="checked"/> 전체 선택</label>
							...
							<tbody>
								<c:forEach var="cDTO" items="${list }" varStatus="status">
									<!-- 수량 변경 -->
										<tr>
											<td>
												<input type="checkbox" id="chk" class="chk" name="chk" value="${cDTO.cNo }" checked="checked" /> <!-- 선택 박스 하나라도 미클릭시 전체선택 체크박스 해제 -->
											</td>
											<td>
											...
											<td>
												...
												<span class="productTotalPrice" id="cPrice${cDTO.cNo }"><fmt:formatNumber value="${cDTO.productTotalPrice }" pattern="#,##0"/></span>
												<input type="hidden" name="cNo" value="${cDTO.cNo }" /> <!-- 바로 주문용 장바구니 번호 -->
												 <!-- 수정2. 상품 가격 pDisrate 값 추가 -->
												<input type="hidden" id="pPrice${cDTO.cNo }" name="pPrice" value="${cDTO.pPrice * (1-(cDTO.pDisrate/100)) }" /> <!-- 상품 가격 -->
												<input type="button" value="바로주문" onclick="fn_cOrderQuick(this.form)" />
											</td>
										</tr>
										<script type="text/javascript">
											$(document).ready(function(){
												...
												// var inputId = '#plusBtn' + i + '~input[type="text"]';
												// 전체선택 해제여부
												$("#chk").click(function() {
												...
												$('#plusBtn'+i).click( function(){
													if($('#cAmount'+i).val() >= 10) {
														alert('1보다 작고 10보다 큰 수는 넣을수 없습니다.');
														return;
													}
